...
<body>
	...
	<div id="main">
		...
		<c:if test="${empty sessionScope.cart.cartItems }">
			<h1 align="center">
				购物车暂无数据，快来<a href="index.jsp" style="color: red">购物</a>呀
			</h1>
		</c:if>
		...
		<c:if test="${not empty sessionScope.cart.cartItems }">
			<table>
				...
				<!-- 这里不用cart.getCartItems，而是直接cart.CartItems，get方法后面是接属性，
	注意开头字母要改小写，可以这样用 -->
				<c:forEach var="cartItem" items="${sessionScope.cart.cartItems }">
					<tr>
						<td>${cartItem.book.title }</td>
						<%-- 					<td>${cartItem.count }</td> --%>
						<td><input id="${cartItem.book.id}" type="text" class="cartItemCount" name="${cartItem.book.stock}"
							value="${cartItem.count }" size="2" style="text-align: center">
						</td>

						<td>${cartItem.book.price  }</td>
						<td>${cartItem.amount }</td>
						<td><a
							href="CartServlet?method=delCartItem&bookId=${cartItem.book.id }">删除</a></td>
					</tr>
				</c:forEach>
...
<!-- 			<div class="cart_info"> -->
				<span class="cart_span">购物车中共有<span class="b_count">${sessionScope.cart.totalCount }</span>件商品
					...
					href="CartServlet?method=emptyCart">清空购物车</a></span> <span
